...
function Bot({ profile, onSession }: { profile: string; onSession: (id: string) => void }) {
...
function Sessions({ activeSession }: { activeSession: string }) { return <section className="placeholder-screen"><p className="eyebrow">SESSIONS</p><h2>Keep the thread.</h2><p>Recent Hermes sessions will appear here. The current session is {activeSession ? <code>{activeSession.slice(0, 12)}…</code> : 'not started yet'}.</p><div className="notice-card"><strong>Next slice</strong><span>Search, resume, fork, and rename sessions without leaving the mobile flow.</span></div></section>; }
...
function App() {
  ...
  const screen = useMemo(() => ({ bot: <Bot profile={profile} onSession={setSession} />, sessions: <Sessions activeSession={session} />, activity: <Activity />, settings: <Settings connected={connected} /> }[tab]), [connected, session, tab]);
