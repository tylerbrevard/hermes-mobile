...
function Bots({ selected, onSelect, onBack }: { selected: string; onSelect: (profile: ProfileInfo) => void; onBack: () => void }) {
  ...
  return <section className="command-center">
    <header className="command-header"><button className="icon-button back-button" onClick={onBack} aria-label="Back to chat">‹</button><div className="brand-lockup"><span className="brand-orbit">⌁</span><div><p className="eyebrow">HERMES</p><strong>Agents</strong></div></div><span className="live-pill"><i /> {ready.length} online</span></header>
...
function Sessions({ profile, activeSession, onOpen, onBack }: { profile: string; activeSession: string; onOpen: (id: string, profile?: string) => void; onBack: () => void }) {
...
function NavSheet({ open, onClose, activeProfile, onProfile, onNavigate, onNewChat }: { open: boolean; onClose: () => void; activeProfile: string; onProfile: (profile: ProfileInfo) => void; onNavigate: (tab: Tab) => void; onNewChat: () => void }) {
  const [profiles, setProfiles] = useState<ProfileInfo[]>([]);
  useEffect(() => { if (open) api<{ data?: ProfileInfo[] }>('/api/profiles').then((body) => setProfiles(body.data ?? [])).catch(() => setProfiles([])); }, [open]);
  const ready = profiles.filter((profile) => profile.active);
  const current = profiles.find((profile) => profile.id === activeProfile);
  return <div className={`nav-sheet ${open ? 'open' : ''}`} aria-hidden={!open}>
    <button className="nav-sheet-scrim" onClick={onClose} aria-label="Close menu" tabIndex={open ? 0 : -1} />
    <div className="nav-sheet-panel" role="dialog" aria-modal="true" aria-label="Navigation">
      <div className="nav-sheet-handle" />
      <div className="nav-sheet-head"><div><p className="eyebrow">HERMES MOBILE</p><h2>{current?.name ?? 'Your agents'}</h2></div><button className="icon-button" onClick={onClose} aria-label="Close">✕</button></div>
      <div className="nav-sheet-bots" role="list" aria-label="Switch bot">{ready.map((profile) => <button role="listitem" key={profile.id} className={profile.id === activeProfile ? 'active' : ''} onClick={() => onProfile(profile)} aria-label={`Switch to ${profile.name}`}><span className={`bot-avatar tone-${profileTone(profile.id)}`}>{profileMark(profile)}<i /></span><small>{profile.name}</small></button>)}</div>
      <nav className="nav-sheet-list">
        <button onClick={onNewChat}><span>New chat</span><b>＋</b></button>
        <button onClick={() => onNavigate('sessions')}><span>Sessions</span><b>›</b></button>
        <button onClick={() => onNavigate('activity')}><span>Activity</span><b>›</b></button>
        <button onClick={() => onNavigate('settings')}><span>Settings</span><b>›</b></button>
      </nav>
    </div>
  </div>;
}
...
function App() {
  const [paired, setPaired] = useState<boolean | null>(null); const [tab, setTab] = useState<Tab>('bot'); const [menuOpen, setMenuOpen] = useState(false); const [connected, setConnected] = useState(false); const [session, setSession] = useState(''); const [activeProfile, setActiveProfile] = useState('default'); const [records, setRecords] = useState<RunRecord[]>(() => parseRunRecords(localStorage.getItem('hermes-mobile:runs')));
  ...
  function navigate(next: Tab) { setTab(next); setMenuOpen(false); }
  function openSession(id: string, profile = activeProfile) { if (profile !== activeProfile) setActiveProfile(profile); setSession(id); setTab('bot'); setMenuOpen(false); }
  function selectProfile(profile: ProfileInfo) { if (!profile.active) return; setActiveProfile(profile.id); setSession(''); setTab('bot'); setMenuOpen(false); }
  function newChat() { setSession(''); setTab('bot'); setMenuOpen(false); }
  const screen = useMemo(() => ({ bot: <Bot profile={activeProfile} activeSession={session} onSession={setSession} onRun={saveRun} onMenu={() => setMenuOpen(true)} />, bots: <Bots selected={activeProfile} onSelect={selectProfile} onBack={() => navigate('bot')} />, sessions: <Sessions profile={activeProfile} activeSession={session} onOpen={openSession} onBack={() => navigate('bot')} />, activity: <Activity records={records} onOpen={openSession} onRun={saveRun} onBack={() => navigate('bot')} />, settings: <Settings connected={connected} profile={activeProfile} onProfile={selectProfile} onClearActivity={clearActivity} onBack={() => navigate('bot')} /> }[tab]), [activeProfile, connected, records, session, tab]);
  ...
  return <main className="app-shell">{screen}<NavSheet open={menuOpen} onClose={() => setMenuOpen(false)} activeProfile={activeProfile} onProfile={selectProfile} onNavigate={navigate} onNewChat={newChat} /></main>;
}
